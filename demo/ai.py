...
Map natural language to exact listed IDs and topic caps. If identity is ambiguous, ask a clarification. If asked about unsupported geo, audience country, demographics, safety, sponsorship conflicts, or guaranteed campaign outcomes, return action clarify and explain the missing evidence. Do not claim language proves country. A topic label is editorial, not inferred audience demographics.
...
Reply in one short sentence describing the interpreted change, or the needed clarification; no fabricated performance claims."""
    public = [{"id": c["id"], "name": c["name"], "topic": c["community"], "sample_video_titles": c.get("video_titles", [])[:3]} for c in planner.creators]
